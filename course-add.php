...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // NOTE: duplicate course_code check removed intentionally —
    // multiple courses can now share the same course code.

    if (empty($errors)) {
        $stmt = $pdo->prepare("
            INSERT INTO courses (course_code, course_name, description, duration, course_fee, status)
            VALUES (?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([$code, $name, $desc ?: null, $duration ?: null, $fee, $status]);
        $success = true;
        // clear form
        $_POST = [];
    }
...
.form-label { font-weight:600; font-size:.85rem; margin-bottom:.35rem; color:var(--ink); }
...
<div class="page-head">
    <h1><i class="bi bi-plus-circle"></i> Add Course</h1>
</div>

<?php if ($success): ?>
    <div class="alert alert-success">Course added successfully! <a href="courses.php">View all courses</a></div>
<?php endif; ?>
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input type="text" name="course_code" class="form-control"
                       ...
                       placeholder="e.g. DBM, HND, CIM" required>
            </div>
            <div class="col-md-6">
                <label class="form-label">Course Name *</label>
                <input type="text" name="course_name" class="form-control"
                       value="<?= htmlspecialchars($_POST['course_name'] ?? '') ?>"
                       placeholder="e.g. Diploma in Business Management" required>
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                ...
                <input type="number" name="course_fee" class="form-control" step="0.01" min="0"
                       value="<?= htmlspecialchars($_POST['course_fee'] ?? '0') ?>">
            </div>
            <div class="col-12">
            ...
            <div class="col-12 d-flex gap-2 mt-3">
                <button type="submit" class="btn-save"><i class="bi bi-check-lg"></i> Save Course</button>
